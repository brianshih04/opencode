              ...
              try {
                  ...
                  return {
                    isVision: body.messages.some(
                      (msg: { content?: Array<{ type: string }> }) =>
                        Array.isArray(msg.content) && msg.content.some((part: { type: string; content?: Array<{ type: string }> }) => part.type === "image_url"),
                    ),
                  ...
                  return {
                    isVision: body.input.some(
                      (item: { content?: Array<{ type: string }> }) =>
                        Array.isArray(item?.content) && item.content.some((part: { type: string; content?: Array<{ type: string }> }) => part.type === "input_image"),
                    ),
                    isAgent: last?.role !== "user",
                  }
                }
                ...
                if (body?.messages) {
                  ...
                  const hasNonToolCalls =
                    Array.isArray(last?.content) && last.content.some((part: { type: string; content?: Array<{ type: string }> }) => part?.type !== "tool_result")
                  return {
                    isVision: body.messages.some(
                      (item: { content?: Array<{ type: string }> }) =>
